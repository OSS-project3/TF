...
| 서비스 | 주소 |
|--------|------|
| 프론트엔드 | [http://113-198-66-75.nip.io:13137](http://113-198-66-75.nip.io:13137) |
| 백엔드 API | http://113-198-66-75.nip.io:13137/api/v1 (nginx 프록시) |
| Swagger UI | http://113.198.66.75:10137/swagger-ui/index.html#/ |
...
로그인/회원가입 화면에서 **Google로 계속하기** 버튼으로 소셜 로그인을 지원합니다.

- 기존 계정(이메일 일치)이 있으면 바로 로그인
- 신규 계정이면 자동 가입 후 **역할 선택 화면**(`/setup-role`)으로 이동
- 초대 링크(`?token=...`)와 함께 사용하면 Google 가입과 동시에 워크스페이스 합류

**설정 방법:**

1. [Google Cloud Console](https://console.cloud.google.com/) → OAuth 2.0 클라이언트 생성
2. **승인된 JavaScript 원본** 추가 (IP 주소 불가, 도메인 필요):
   ```
   http://localhost:3000
   http://113-198-66-75.nip.io:13137
   ```
3. `.env`와 `frontend/.env`에 Client ID 설정:
   ```bash
   # TF/.env
   GOOGLE_CLIENT_ID=your-client-id.apps.googleusercontent.com
   VITE_GOOGLE_CLIENT_ID=your-client-id.apps.googleusercontent.com

   # TF/frontend/.env  ← Vite 빌드용, 별도 필요
   VITE_GOOGLE_CLIENT_ID=your-client-id.apps.googleusercontent.com
   ```
4. 프론트 재빌드 + Docker 재시작:
   ```bash
   cd frontend && npm run build && cd .. && sudo docker compose up --build -d
   ```

> Client Secret은 불필요합니다. `@react-oauth/google`은 팝업(GIS) 방식으로 동작하며 백엔드는 Google tokeninfo API로 ID 토큰만 검증합니다.
...
- **인증** — 이메일/비밀번호 회원가입·로그인·로그아웃 (JWT) + **Google OAuth 로그인**
- **프로젝트** — 목록·상세·생성, 멤버 관리, 진행률·health 자동 집계
- **태스크** — 보드/일정, 상태·**다중 담당자**·브랜치 연결, 내 작업, GitHub PR 머지 웹훅 연동
...
- **온보딩** — 최초 로그인 후 기술 스택(칩 UI)·주간 가용 시간 설정 (`/setup-profile`), Google 신규 가입 시 역할 선택 (`/setup-role`)
- **워크스페이스/초대** — 워크스페이스 단위 데이터 격리, 초대 링크 생성·수락
- **설정** — 프로필·기술 스택(칩 UI)·주간 가용 시간·비밀번호 변경·회원 탈퇴·팀원 초대
- **관리자** — 멤버 목록(PM 전용)
...
| `GOOGLE_CLIENT_ID` | .env | (빈 값) | Google OAuth 클라이언트 ID. 미설정 시 Google 로그인 비활성 |
| `VITE_GOOGLE_CLIENT_ID` | frontend/.env | (빈 값) | Vite 빌드용 Google 클라이언트 ID. `TF/.env`와 별도로 `frontend/.env`에도 필요 |
